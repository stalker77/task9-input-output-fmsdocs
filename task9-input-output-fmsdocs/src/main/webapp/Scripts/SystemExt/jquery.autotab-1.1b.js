...
$.fn.autotab = function(options) {
  ...
  //For jquery ui multiselect:
	if (this && this.get(0).nextSibling.className && this.get(0).nextSibling.className.indexOf("ui-multiselect") != -1) {
	  console.log($('#' + defaults.target.get(0).id + 'Button'));
	  $('#' + defaults.target.get(0).id + 'Button').off('keyup');
    // Go to the previous element when backspace
    // is pressed in an empty input field
    /*return $('#' + defaults.target.get(0).id + 'Button').bind('keydown', function (e) {
      ...
      if (e.which != 8) {
        ...
        if ($.inArray(e.which, keys) == -1 && ((val.length == defaults.maxlength) || (e.which == 13)) && defaults.target)
    ...
    return $(this).bind('keydown', function(e) {
      ...
      if (e.which != 8) {
        ...
        /*console.log(defaults.target);*/
        if ($.inArray(e.which, keys) == -1 && e.which == 13 && defaults.target && defaults.target.get(0).nextSibling.className && defaults.target.get(0).nextSibling.className.indexOf("ui-multiselect") != -1) {
          document.getElementById(defaults.target.get(0).id + 'Button').focus();
          return;
        }
